 ...
 */
class Stats {
  private perUser = new Map<string, { requests: number; totalDurationMs: number; containers: number }>();

  record(userId: string, durationMs: number, hasContainer: boolean): void {
    const cur =
      this.perUser.get(userId) ?? { requests: 0, totalDurationMs: 0, containers: 0 };
    cur.requests += 1;
