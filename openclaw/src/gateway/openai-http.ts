...
import type { AaasContext, ImageContent } from "../agents/command/types.js";
...
function resolveAaasResponseToolCalls(
  aaas: AaasContext,
  meta: unknown,
): Array<{ id: string; name: string }> | undefined {
  const { stopReason, pendingToolCalls } = resolveStopReasonAndPendingToolCalls(meta);
  if (stopReason === "tool_calls" && pendingToolCalls && pendingToolCalls.length > 0) {
    return pendingToolCalls.map((call) => ({ id: call.id, name: call.name }));
  }
  const requestId = aaas.requestId?.trim();
  if (!requestId) {
    return undefined;
  }
  const executed = getAaasToolCallsForRequest(requestId);
  return executed.length > 0 ? executed.map((call) => ({ id: call.id, name: call.name })) : undefined;
}

function setAaasResponseHeaders(
  res: ServerResponse,
  aaas: AaasContext,
  opts?: { containerId?: string; toolCalls?: Array<{ id: string; name: string }> },
): void {
  if (aaas.requestId) {
    res.setHeader("x-aaas-request-id", aaas.requestId);
  }
  if (opts?.containerId) {
    res.setHeader("x-aaas-container-id", opts.containerId);
  }
  if (opts?.toolCalls && opts.toolCalls.length > 0) {
    const toolCallsStr = opts.toolCalls
      .map((call) => `${call.id}:${call.name}`)
      .join(",");
    res.setHeader("x-aaas-tool-calls", toolCallsStr);
  }
}
...
export async function handleOpenAiHttpRequest(
  ...
  const { agentId, sessionKey, messageChannel, aaas } = resolveGatewayRequestContext({
    req,
    model,
    user,
  ...
  if (modelError) {
    setAaasResponseHeaders(res, aaas, {toolCalls: aaasToolCalls ?? pendingToolCalls, containerId: resolveAaasContainerId(aaas),});
    sendJson(res, 400, {
  ...
  const aaasRequestId = aaas.requestId?.trim();
  if (aaasRequestId) {
    resetAaasToolCallTrackingForRequest(aaasRequestId);
  }
  ...
  if (!stream) {
    ...
    try {
      ...
      const { stopReason, pendingToolCalls } = resolveStopReasonAndPendingToolCalls(meta);
      const aaasToolCalls = resolveAaasResponseToolCalls(aaas, meta);
