...
export function getAaasContext(req: IncomingMessage): AaasContext {
  return {
    userId: getHeader(req, "x-aaas-user-id")?.toString(),
    workspace: getHeader(req, "x-aaas-workspace")?.toString(),
    authProfileId: getHeader(req, "x-aaas-auth-profile")?.toString(),
    requestId: getHeader(req, "x-aaas-request-id")?.toString(),
  };
}
...
export function resolveGatewayRequestContext(params: {
  ...
  const aaas = getAaasContext(params.req);

  return { agentId, sessionKey, messageChannel, aaas };
}
