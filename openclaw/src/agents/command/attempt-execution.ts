...
import type { AaasContext } from "../other/http-utils.js";
...
const DEFAULT_AAAS_TOOLS_LOG_PATH = "/app/logs/openclaw-tools.jsonl";

type AaasToolLogEvent = {
  ts: string;
  requestId: string;
  phase: "start" | "end";
  runId: string;
...
type AgentAttemptAgentEvent = {
  stream: string;
  data?: Record<string, unknown>;
  sessionKey?: string;
};

const aaasToolsLogWriters = new Map<string, ReturnType<typeof getQueuedFileWriter>>();
const aaasToolCallStartTimes = new Map<string, number>();
const aaasToolCallsByRequestId = new Map<string, Array<{ id: string; name: string }>>();
const aaasToolCallCountByRequestId = new Map<string, number>();

function resolveAaasToolsLogPath(env: NodeJS.ProcessEnv = process.env): string {
  const override = env.OPENCLAW_AAAS_TOOLS_LOG_FILE?.trim();
  return override || DEFAULT_AAAS_TOOLS_LOG_PATH;
}

function resolveAaasRequestId(opts: AgentCommandOpts): string | undefined {
  return opts.aaas?.requestId?.trim();
}

function buildAaasToolTimingKey(requestId: string, toolCallId: string): string {
  return `${requestId}\u0000${toolCallId}`;
}

function appendAaasToolLog(event: AaasToolLogEvent): void {
  const line = safeJsonStringify(event);
  if (!line) {
    return;
  }
  getQueuedFileWriter(aaasToolsLogWriters, resolveAaasToolsLogPath()).write(`${line}\n`);
}

function recordAaasToolCallStart(params: {
  requestId: string;
  runId: string;
  toolCallId: string;
  name: string;
  ...
  aaasToolCallStartTimes.set(buildAaasToolTimingKey(requestId, toolCallId), Date.now());
  const nextCount = (aaasToolCallCountByRequestId.get(requestId) ?? 0) + 1;
  aaasToolCallCountByRequestId.set(requestId, nextCount);

  const toolCalls = aaasToolCallsByRequestId.get(requestId) ?? [];
  toolCalls.push({ id: toolCallId, name });
  aaasToolCallsByRequestId.set(requestId, toolCalls);
  ...
  appendAaasToolLog({
    ...
    phase: "start",
    runId: params.runId,
    toolCallId: sanitizeForLog(toolCallId),
    name: sanitizeForLog(name),
  });
}

function recordAaasToolCallEnd(params: {
  requestId: string;
  runId: string;
  toolCallId: string;
  name: string;
  ...
  const timingKey = buildAaasToolTimingKey(requestId, toolCallId);
  const startedAt = aaasToolCallStartTimes.get(timingKey);
  aaasToolCallStartTimes.delete(timingKey);
...
/** Clears per-request executed-tool tracking before a new AaaS HTTP run. */
export function resetAaasToolCallTrackingForRequest(requestId: string): void {
  ...
  aaasContainerIdByRequestId.delete(normalized);
...
/** Tool-call count for an AaaS request (`x-aaas-tool-calls` accounting). */
export function getAaasToolCallCountForRequest(requestId: string): number {
  const normalized = requestId.trim();
  if (!normalized) {
    return 0;
  }
  return aaasToolCallCountByRequestId.get(normalized) ?? 0;
}

/** Tool-call entries for an AaaS request (`id:name` header segments). */
export function getAaasToolCallsForRequest(
  requestId: string,
  ...
  return aaasToolCallsByRequestId.get(normalized) ?? [];
}

function createAaasToolUsageOnAgentEvent(params: {
  requestId?: string;
  runId: string;
  onAgentEvent: (evt: AgentAttemptAgentEvent) => void;
}): (evt: AgentAttemptAgentEvent) => void {
  const requestId = params.requestId?.trim();
  if (!requestId) {
    return params.onAgentEvent;
  }
  ...
  return (evt) => {
    if (evt.stream === "tool") {
      ...
      const toolCallId =
        typeof evt.data?.toolCallId === "string" ? evt.data.toolCallId.trim() : "";
      const name = typeof evt.data?.name === "string" ? evt.data.name.trim() : "unknown";
      if (phase === "start" && toolCallId) {
        recordAaasToolCallStart({
          requestId,
          runId: params.runId,
          toolCallId,
          name,
...
export function runAgentAttempt(params: {
  ...
  const embeddedPiHarnessOverride =
    ...
    (agentHarnessPolicy.runtime === "pi" && embeddedPiProvider !== params.providerOverride
      ? "pi"
      : undefined);
  const aaasRequestId = resolveAaasRequestId(params.opts);
  const onAgentEvent = createAaasToolUsageOnAgentEvent({
    requestId: aaasRequestId,
    runId: params.runId,
...
export function setAaasContainerIdForRequest(requestId: string, containerId: string): void {
  const normalized = requestId.trim();
  if (!normalized || !containerId.trim()) return;
