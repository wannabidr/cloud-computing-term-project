...
export async function resolveApiKeyForProvider(params: {
  ...
  /** AaaS-provided auth profile ID (takes highest priority when present) */
  aaasAuthProfileId?: string;
}): Promise<ResolvedProviderAuth> {
  const { provider, cfg, preferredProfile } = params;
  
  // Priority 1: AaaS-provided auth profile ID (always treat as locked)
  const effectiveProfileId = params.aaasAuthProfileId ?? params.profileId;
  const isAaasProfile = Boolean(params.aaasAuthProfileId);
  
  let scopedStore: AuthProfileStore | undefined = params.store;

  if (effectiveProfileId) {
    const awsSdkProfileAuth = resolveConfiguredAwsSdkProfileAuth({ cfg, provider, profileId: effectiveProfileId });
    ...
    if (!resolved) {
      throw new Error(`No credentials found for profile "${effectiveProfileId}".`);
    }
    const resolvedProfileId = resolved.profileId ?? effectiveProfileId;
    ...
    // the caller has not locked this profile AND it's not an AaaS profile,
    // fall through to env/config resolution so provider-owned real credentials
    // take precedence. For AaaS profiles, always respect the provided profileId.
    if (
      !isAaasProfile &&
...
export async function hasAvailableAuthForProvider(params: {
  ...
  // AaaS profile ID is immediately valid
  if (params.aaasAuthProfileId) {
    return true;
  }
...
export async function getApiKeyForModel(params: {
  ...
  return resolveApiKeyForProvider({
    ...
    aaasAuthProfileId: params.aaasAuthProfileId,
  });
}
